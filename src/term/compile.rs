...
impl Term {
  ...
  pub fn to_hvm2(&self, env: im::Vector<String>, met: &mut usize) -> String {
    fn binder(name: &str) -> String {
      format!("x{}", name.replace("-", "._."))
    }
    match self {
      Term::All { nam, inp, bod } => {
        let inp = inp.to_hvm2(env.clone(), met);
        let bod = bod.to_hvm2(cons(&env, nam.clone()), met);
        format!("(All \"{}\" {} λ{} {})", nam, inp, binder(nam), bod)
      }
      Term::Lam { nam, bod } => {
        let bod = bod.to_hvm2(cons(&env, nam.clone()), met);
      ...
      Term::App { fun, arg } => {
        let fun = fun.to_hvm2(env.clone(), met);
        let arg = arg.to_hvm2(env.clone(), met);
        format!("(App {} {})", fun, arg)
      }
      Term::Ann { chk, val, typ } => {
        let val = val.to_hvm2(env.clone(), met);
        let typ = typ.to_hvm2(env.clone(), met);
      ...
      Term::Op2 { opr, fst, snd } => {
        let fst = fst.to_hvm2(env.clone(), met);
        let snd = snd.to_hvm2(env.clone(), met);
        format!("(Op2 {} {} {})", opr.to_hvm2(), fst, snd)
      }
      Term::Mat { nam, x, z, s, p } => {
        let x = x.to_hvm2(env.clone(), met);
        let z = z.to_hvm2(env.clone(), met);
        let s = s.to_hvm2(cons(&env, format!("{}-1", nam)), met);
        let p = p.to_hvm2(cons(&env, nam.clone()), met);
        format!("(Mat \"{}\" {} {} λ{} {} λ{} {})", nam, x, z, binder(&format!("{}-1", nam)), s, binder(nam), p)
      }
      Term::Let { nam, val, bod } => {
        let val = val.to_hvm2(env.clone(), met);
