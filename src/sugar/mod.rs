...
impl Display for ADT {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    if !self.pars.is_empty() {
      write!(f, " {}", self.pars.join(" "))?;
    }

    if !self.idxs.is_empty() {
      write!(f, " {}", self.idxs.iter().map(|(nam, typ)| format!("({nam}: {typ})")).collect::<Vec<_>>().join(" "))?;
    }
...
impl Display for Match {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "{keyword} {name} = {expr} {{", keyword = if self.fold { "fold" } else { "match" }, name = self.name, expr = self.expr)?;
